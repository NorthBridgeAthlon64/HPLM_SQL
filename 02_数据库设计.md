...
| position     | VARCHAR(50) | 是      | NULL | 元器件在PCB上的位置          |
...
| total_cost        | DECIMAL(12,2) | 是      | NULL | 小计总价                        |
...
CREATE TABLE ProductFeedback (
    ...
    UNIQUE(version_id, customer_id, feedback_date)       -- 同一客户对同一版本每天只能评价一次
);
...
-- FROM RepairRecord r
-- JOIN ProductVersion pv ON r.version_id = pv.version_id
-- JOIN ProductionBatch pb ON pv.version_id = pb.version_id
-- JOIN BatchMaterial bm ON pb.batch_id = bm.batch_id
-- JOIN Component c ON bm.component_id = c.component_id
